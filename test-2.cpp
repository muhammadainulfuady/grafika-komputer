...
bool is_isometric = false; // Mode View
float mazeRotX = -60.0f;   // Kemiringan Isometrik
float mazeRotZ = 45.0f;

int nimRotAxis = 3;  // Sumbu putar NIM
float nimAngle = 0.0f;
float nimOx = -4.0f, nimOy = -5.0f; // Posisi NIM

// ================= LOGIKA TABRAKAN & POSISI (MANUAL TANPA LOOP) =================
...
// Cek keamanan per-kotak sel agar NIM random menempati lorong kosong
bool isCellEmpty(float cx, float cy) {
    ...
    // Jangan nimpa posisi awal player
...
void drawNIM3D() {
    float d = 1.5f;
    glColor3f(0.0f, 0.0f, 1.0f); // NIM Biru Terang
...
void drawNIM2D() {
    glColor3f(0.0f, 0.0f, 1.0f); // NIM Biru Datar
...
void display() {
    ...
    // 1. Gambar Dinding Sepenuhnya Manual (Tanpa Looping)
    ...
    if (rencent_maze == 1) {
        drawMazeWall(-2.0, -3.0, 0.0, -1.0, -1.0, 1.0);
        drawMazeWall(-2.0, -4.0, 0.0,  0.0, -3.0, 1.0);
        drawMazeWall( 2.0, -4.0, 0.0,  4.0, -3.0, 1.0);
        drawMazeWall( 2.0, -2.0, 0.0,  3.0,  3.0, 1.0);
        drawMazeWall( 0.0, -2.0, 0.0,  3.0, -1.0, 1.0);
        drawMazeWall( 0.0, -5.0, 0.0,  1.0, -1.0, 1.0);
        drawMazeWall(-2.0,  2.0, 0.0,  0.0,  3.0, 1.0);
        drawMazeWall( 0.0,  0.0, 0.0,  1.0,  3.0, 1.0);
        drawMazeWall(-4.0,  0.0, 0.0,  1.0,  1.0, 1.0);
        drawMazeWall(-4.0, -5.0, 0.0, -3.0,  1.0, 1.0);
        drawMazeWall(-5.0,  2.0, 0.0, -3.0,  3.0, 1.0);
    } else {
        drawMazeWall( 3.0,  0.0, 0.0,  4.0,  1.0, 1.0);
        drawMazeWall( 2.0,  0.0, 0.0,  3.0,  2.0, 1.0);
        drawMazeWall( 0.0,  2.0, 0.0,  3.0,  3.0, 1.0);
        drawMazeWall(-4.0, -4.0, 0.0, -2.0, -3.0, 1.0);
        drawMazeWall(-2.0, -5.0, 0.0, -1.0, -3.0, 1.0);
        drawMazeWall( 2.0, -4.0, 0.0,  4.0, -3.0, 1.0);
        drawMazeWall( 0.0, -5.0, 0.0,  1.0,  1.0, 1.0);
        drawMazeWall(-3.0, -2.0, 0.0,  3.0, -1.0, 1.0);
        drawMazeWall(-4.0, -2.0, 0.0, -3.0,  2.0, 1.0);
        drawMazeWall(-4.0,  2.0, 0.0, -2.0,  3.0, 1.0);
        drawMazeWall(-2.0,  0.0, 0.0, -1.0,  3.0, 1.0);
    }

    // 2. Gambar NIM dengan kondisi 2D vs 3D
    glPushMatrix();
    glTranslatef(nimOx + 0.5f, nimOy + 0.5f, 0.5f);

    if (is_isometric) {
        // Jika 3D: Berputar otomatis & memakai drawNIM3D
        ...
        // Jika 2D (Default): Diam & datar memakai drawNIM2D
